                    ...
                    {currentItems.map((invoice) => (
                      <tr key={invoice.id}>
                        ...
                        <td className="flex items-center px-6 py-4 whitespace-no-wrap text-center text-sm">
                          ...
                          <button
                            className="mr-2"
                            onClick={() => { setIsView(false); setInvoice(invoice); }}
                          >
                            <FontAwesomeIcon
                              icon={faEye}
                              className="text-cyan-600 text-xl"
        ...
        !isView && <>
          ...
          <div className="bg-white mt-3 border-2 border-cyan-500 p-4 w-[70%] rounded-lg shadow-lg">
            <table className="w-[100%] bg-white">
              <tbody>
                <tr>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">Invoice No</th>
                  <td className="text-left px-6 py-4 whitespace-no-wrap text-sm">{invoice.invoiceNo}</td>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">Date Sent On</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">
                    {new Date(invoice.dateSentOn).toLocaleDateString("es-CL")}</td>
                </tr>
                <tr>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">Amount</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">{invoice.amount}</td>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">GRN Number</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">{invoice.grnNumber}</td>
                </tr>
                <tr>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">PO Number</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">{invoice.poNumber}</td>
                  <th className="text-left px-6 py-3  leading-4 text-gray-600 tracking-wider">Status</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">
                    {
                      invoice.s == "Rejected" ?
                        <span className="bg-red-300 py-1 px-3 rounded text-red-700">Rejected</span>
                ...
                <tr>
                  <th className="text-left px-6 py-3   leading-4 text-gray-600 tracking-wider">Payment Status</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">
                    {
                      invoice.paymentStatus == "Unpaid" ?
                        <span className="bg-yellow-300 py-1 px-3 rounded text-yellow-700">UnPaid</span>
                  ...
                  <th className="text-left px-6 py-3   leading-4 text-gray-600 tracking-wider">Due Date</th>
                  <td className="px-6 py-4 whitespace-no-wrap text-left  text-sm">
                    {new Date(invoice.dueDate).toLocaleDateString("es-CL")}
                  </td>
                </tr>
