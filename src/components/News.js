...
const News = () => {
  ...
  const handleBackwardClick = () => {
    ...
    handleOverflow(
      news[(currentNewsIndex - 1 + news.length) % news.length].content
    );
  };
  ...
  return (
    <div
      className="container mr-10 relative  w-full h-[300px] shadow-2xl"
      onMouseEnter={() => setArrows(false)}
      onMouseLeave={() => setArrows(true)}
    >
      <img
        src={news[currentNewsIndex].imagePath}
        alt=""
      ...
      >
        <p
          className={`text-1xl  text-gray-800 mb-4 px-6 py-2 text-wrap`}
          dangerouslySetInnerHTML={{ __html: news[currentNewsIndex].content }}
        />
      </div>
      <div
        hidden={arrows}
